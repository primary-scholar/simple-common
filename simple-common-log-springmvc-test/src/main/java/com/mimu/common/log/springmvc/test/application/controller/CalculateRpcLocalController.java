...
import com.mimu.common.log.springmvc.test.application.service.CalculateRpcLocalHttpService;
...
public class CalculateRpcLocalController {
    ...
    public RpcResult calculateNumberExecute(NumberOperationParam param) {
        LOGGER.info("get param:{}", JSONObject.toJSONString(param));
        Future<NumberSeedResult> seedResultFuture =
                executorService.submit(() -> operationHttpService.numberSeedGet(param));
        ...
        NumberSeedResult second = JSONObject.parseObject(JSONObject.toJSONString(first), NumberSeedResult.class);
        NumberCalculateParam operationParam = NumberOperationBuilder.build(first, second, param);
        Future<NumberCalculateResult> calculateResultFuture =
                executorService.submit(() -> operationHttpService.numberOperationPost(operationParam));
    ...
    public RpcResult calculateNumberExecuteWrapper(NumberOperationParam param) {
        ...
        TraceContextSnapshot snapshotF = TraceContextManager.capture();
        Future<NumberSeedResult> firstFuture =
                executorService.submit(new CallableWrapper<>(() -> operationHttpService.numberSeedGet(param),
                        snapshotF));
        try {
        ...
        TraceContextSnapshot snapshotS = TraceContextManager.capture();
        Future<NumberSeedResult> secondFuture =
                executorService.submit(new CallableWrapper<>(() -> operationHttpService.numberSeedGet(param),
                        snapshotS));
        try {
        ...
        TraceContextSnapshot snapshotT = TraceContextManager.capture();
        Future<NumberCalculateResult> calculateResultFuture =
                executorService.submit(new CallableWrapper<>(() -> operationHttpService.numberOperationPost(operationParam), snapshotT));
        NumberCalculateResult operationResult = null;
        try {
            operationResult = calculateResultFuture.get(waitSeconds, TimeUnit.SECONDS);
        } catch (InterruptedException | ExecutionException | TimeoutException e) {
    ...
    public RpcResult calculateNumberCompleteFuture(NumberOperationParam param) {
        ...
        AtomicReference<NumberCalculateResult> thirdRef = new AtomicReference<>();
        RpcResult result =
                CompletableFuture.supplyAsync(() -> operationHttpService.numberOperationPost(operationParam)).thenAcceptAsync(numberCalculateResult -> thirdRef.set(numberCalculateResult)).thenApply((Function<Void, RpcResult>) unused -> Objects.isNull(thirdRef.get()) ? RpcResultUtil.fail() : RpcResultUtil.success(thirdRef.get())).join();
        return result;
    }
    ...
    public RpcResult calculateNumberCompleteFutureWrapper(NumberOperationParam param) {
        ...
        AtomicReference<NumberSeedResult> secondRef = new AtomicReference<>();
        TraceContextSnapshot snapshotF = TraceContextManager.capture();
        CompletableFuture.supplyAsync(new SupplierWrapper<>(() -> operationHttpService.numberSeedGet(param),
                snapshotF)).thenAcceptAsync(new ConsumerWrapper<>(numberSeedResult -> firstRef.set(numberSeedResult),
                snapshotF)).join();
        TraceContextSnapshot snapshotS = TraceContextManager.capture();
        CompletableFuture.supplyAsync(new SupplierWrapper<>(() -> operationHttpService.numberSeedGet(param),
                snapshotS)).thenAcceptAsync(new ConsumerWrapper<>(numberSeedResult -> secondRef.set(numberSeedResult)
                , snapshotS)).join();
        NumberSeedResult first = firstRef.get();
        NumberSeedResult second = secondRef.get();
        NumberCalculateParam operationParam = NumberOperationBuilder.build(first, second, param);
        ...
        TraceContextSnapshot snapshotT = TraceContextManager.capture();
        RpcResult result =
                CompletableFuture.supplyAsync(new SupplierWrapper<>(() -> operationHttpService.numberOperationPost(operationParam), snapshotT)).thenAcceptAsync(new ConsumerWrapper<>(numberCalculateResult -> thirdRef.set(numberCalculateResult), snapshotT)).thenApply(new FunctionWrapper<>((Function<Void, RpcResult>) unused -> Objects.isNull(thirdRef.get()) ? RpcResultUtil.fail() : RpcResultUtil.success(thirdRef.get()), snapshotT)).join();
        return result;
    }
